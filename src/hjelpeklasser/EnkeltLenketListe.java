...
public class EnkeltLenketListe<T> implements Liste<T>, Kø<T>
{
    ...
    public static <T> void sorter(Kø<T> ko, Stakk<T> stakk, Comparator<T> c) {
        int n = ko.antall();
        while (n > 0) {
            stakk.leggInn(ko.taUt());    // kandidat for å være den største

            for (int i = 1; i < n; i++) {
                T verdi = ko.taUt();
                if (c.compare(verdi, stakk.kikk()) > 0) {
                    ko.leggInn(stakk.taUt());   // fant en verdi i køen som var større
                    stakk.leggInn(verdi);       // legger den nye verdien øverst i stakken
                } else {
                    ko.leggInn(verdi);          // verdien var ikke større enn den i stakken
                }
            }
            n--;
        }
        while (!stakk.tom()) {
            ko.leggInn(stakk.taUt());
        }
    }
